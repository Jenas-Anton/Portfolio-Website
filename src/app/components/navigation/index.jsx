        ...
        {({ size }) => {
          return size && size >= 480 ? (
            <motion.div
              ...
              initial="hidden"
              animate="show"
              className="w-max flex items-center justify-center relative" // Remove the extra 'p' class
            >
              {BtnList.map((btn, index) => {
                ...
                const radius = isLarge
                  ...
                  ? "calc(30vw - 5rem)"
                  : "calc(40vw - 5rem)";
                const x = `calc(${radius}*${Math.cos(angleRad)} + 0.5rem)`;
                const y = `calc(${radius}*${Math.sin(angleRad)} + 2rem)`;
