    ...
    this.program = new Program(this.gl, {
      ...
      vertex: `
        ...
        void main() {
          vUv = uv;
          vec3 p = position;
          p.z = 0.0 * (sin(p.x * 4.0 + uTime) * 1.5 + cos(p.y * 2.0 + uTime) * 1.5) * (0.1 + uSpeed * 0.5);
...
this.plane.scale.y =
  (this.viewport.height * (1000 * this.scale * mobileScale)) / this.screen.height;
this.plane.scale.x =
  (this.viewport.width * (1100 * this.scale * mobileScale)) / this.screen.width;
...
class App {
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    const galleryItems = items && items.length ? items : projectsData;
